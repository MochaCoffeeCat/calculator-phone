...
var Calculartor = __webpack_require__(4);

// const layout = new Layout($('#calculator'));
//     layout.build();
var calculartor = new Calculartor($('#calculator'));
calculartor.init();
...
module.exports = function () {
    ...
    _createClass(Calculator, [{
        ...
        key: '_viewAreaEventBind',
        value: function _viewAreaEventBind() {
            var startY = void 0,
        ...
        key: '_operateBtnEventBind',
        value: function _operateBtnEventBind() {
            var self = this;
        ...
        key: '_operateNumInput',
        value: function _operateNumInput(val) {
            var currentObj = void 0;
            for (var i in this.currentOperateArr) {
        ...
        key: '_operateInput',
        value: function _operateInput(val) {
            var currentObj = void 0;
            for (var i in this.currentOperateArr) {
                if (this.currentOperateArr[i].active) {
                    currentObj = this.currentOperateArr[i];
            ...
            if ('+' === val || '*' === val || '/' === val || '-' === val) {
                if ('result' === currentObj.type) {
                    ...
                    if ('0' === currentObj.value && '-' === val || '' === currentObj.value) {
                        ...
                        for (var _i in this.currentOperateArr) {
                            this.currentOperateArr[_i].active = false;
                        }
                ...
                var numStr = currentObj.value;
                currentObj.value = val === '.' ? numStr.indexOf('.') === -1 ? numStr + '.' : numStr : val === '%' ? numStr.indexOf('.') === -1 ? parseInt(numStr) / 100 : parseFloat(numStr) / 100 : numStr.length === 1 ? '0' : numStr.substring(0, numStr.length - 1);
                this._calculatorHandler();
        ...
        key: '_calculatorHandler',
        value: function _calculatorHandler(fromEqual) {
            var origNum = 0,
                resultIndex = -1;
            for (var i in this.currentOperateArr) {
                if (this.currentOperateArr[i].type !== 'result') origNum = calc(origNum, this.currentOperateArr[i]);else resultIndex = i;
            }
            if (resultIndex !== -1) {
                this.currentOperateArr.splice(resultIndex, 1);
            }
            if (fromEqual) {
                for (var _i2 in this.currentOperateArr) {
                    this.currentOperateArr[_i2].active = false;
                }
            }
            this.currentOperateArr.push({
                value: origNum,
            ...
            function calc(orig, operateObj) {
                var result = orig;
                if (!isNaN(parseFloat(operateObj.value))) switch (operateObj.operator) {
                    case '-':
                        {
        ...
        key: '_viewAreaRender',
        value: function _viewAreaRender() {
            var $viewScrollContainer = this._$container.find('#viewScrollContainer'),
                $CurOperateArea = this._$container.find('#currentOperateArea'),
                $HistoryArea = this._$container.find('#historyArea'),
                hisHtml = '',
                curHtml = '';
            for (var i in this.historyArr) {
                var tmp = this.historyArr[i];
            ...
            for (var _i3 in this.currentOperateArr) {
                var _tmp = this.currentOperateArr[_i3];
                curHtml += '<div class="view-row ' + (_tmp.active ? 'active' : '') + '"><span>' + (_tmp.operator === '/' ? '÷' : _tmp.operator === '*' ? '×' : _tmp.operator) + '</span><span>' + _tmp.value + '</span></div>';
            }
